...
function countCertificationsByStatus(person, status) {
  return person.certifications.filter((c) => c.status === status).length;
}
function getDaysUntilExpiration(cert) {
  const now2 = /* @__PURE__ */ new Date();
  const diffTime = cert.expirationDate.getTime() - now2.getTime();
  return Math.ceil(diffTime / (1e3 * 60 * 60 * 24));
}
const now = /* @__PURE__ */ new Date();
const daysAgo = (days) => new Date(now.getTime() - days * 24 * 60 * 60 * 1e3);
const daysFromNow = (days) => new Date(now.getTime() + days * 24 * 60 * 60 * 1e3);
...
function createCertification(id, name, issuedDaysAgo, expiresInDays) {
...
function getAllCertifications() {
  return personnelData.flatMap(
    (person) => person.certifications.map((cert) => ({
      ...cert,
...
function filterPersonnel(data, filters) {
...
function getPersonnelByShift(shift) {
...
const availableShifts = ["Alpha", "Beta", "Gamma"];
const availableCertificationTypes = certificationTypes;
export {
